...
var defaultParams = function defaultParams(options) {
  ...
  return {
    ...
    'Trading': {
      'X-EBAY-API-COMPATIBILITY-LEVEL' : options.version ? options.version : '893',
...
var ebayApiPostXmlRequest = function ebayApiPostXmlRequest(options, callback) {
  ...
  console.log('EBAY API ------> ' + url + ' ------> ' + JSON.stringify(options.reqOptions, null, 4));
  var request = requestModule(
    ...
    async.waterfall([
      ...
      function parseData(data, next) {
        ...
        console.log('HOLY FUCK PARSE DATA ' + JSON.stringify(data));
        next(null, data);
      }
    ...
    function(error, data){
      ...
      console.log('HOLY BANANAS -----> ' + JSON.stringify(data));
      callback(null, data);
    });
